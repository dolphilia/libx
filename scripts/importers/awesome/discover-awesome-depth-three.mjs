...
const candidateLimit =
  decision.decision === 'raise-limit' && Number.isInteger(decision.trigger?.limit)
    ...
    : 1000;
const depth = Number(
  process.argv.find((argument) => argument.startsWith('--depth='))?.slice(8) ?? 3
);
...
const sourceRepositories = new Set(
  previous.newCandidates.map((candidate) => candidate.repository.toLowerCase())
);
...
for (const source of state.visited.filter(
  (candidate) => sourceRepositories.has(candidate.repository.toLowerCase()) && candidate.readmePath
)) {
  sourceDocuments += 1;
...
const newCandidates = unique.filter(
  (candidate) =>
    ...
    !failed.has(candidate.repository.toLowerCase())
);
...
state.checkpoints.push({
  kind: `depth-${depth}-link-extraction`,
