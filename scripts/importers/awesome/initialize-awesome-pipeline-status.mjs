...
const included = lock.sources.filter((source) => source.status === 'included').sort((a, b) => a.sourceId.localeCompare(b.sourceId));
...
for (let index = 0; index < included.length; index += 10) {
  ...
  batches.push({
    ...
    inputHash: sha256(JSON.stringify(sources.map((source) => [source.sourceId, source.documentSha256]))),
    outputHash: sha256(JSON.stringify(sources.map((source) => [source.sourceId, source.documentSha256]))),
    checks: ['awesome:validate-canonical', 'awesome:publish --check'],
    pages: Object.fromEntries(sources.map((source) => [source.sourceId, {
...
console.log(`Initialized Awesome pipeline status (${batches.length} batches, ${included.length} sources)`);
