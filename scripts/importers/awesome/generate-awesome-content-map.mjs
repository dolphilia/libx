...
if (unmapped.length > 0)
  throw new Error(`起点カテゴリを復元できないincludedソースがあります: ${unmapped.map((x) => x.sourceId).join(', ')}`);
...
const map = { generatedAt: new Date().toISOString(), snapshotVersion: lock.snapshotVersion, entries, categories };
...
if (process.argv.includes('--check')) {
  ...
  console.log(`Awesome content map check: OK (${entries.length} sources, ${categories.length} categories)`);
} else {
  writeJsonAtomic(outputJson, map);
  const lines = [
    '# Awesome content map', '',
    '`SOURCES.lock.json`と`DISCOVERY_STATE.json`から生成する、固定済み本文の配置対応表。', '',
    '## 上位カテゴリ', '', '| カテゴリ | included本文数 |', '| --- | ---: |',
    ...categories.map((category) => `| ${category.name} | ${category.sourceCount} |`), '',
    '## ページ対応', '', '| sourceId | repository | 上位カテゴリ |', '| --- | --- | --- |',
    ...entries.map((entry) => `| ${entry.sourceId} | ${entry.repository} | ${entry.categories.join(', ')} |`), '',
  ];
  fs.writeFileSync(outputMarkdown, lines.join('\n'));
  console.log(`Generated Awesome content map (${entries.length} sources, ${categories.length} categories)`);
}
