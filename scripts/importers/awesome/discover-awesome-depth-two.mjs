...
const firstDepthRepositories = new Set(
  firstDepth.candidates.map((candidate) => candidate.repository.toLowerCase())
);
const cachePath = (candidate) =>
  path.join(
    tempDir,
    '01-source/recursive',
    candidate.repository.replace('/', '--'),
...
for (const source of state.visited.filter(
  (candidate) =>
    firstDepthRepositories.has(candidate.repository.toLowerCase()) && candidate.readmePath
)) {
...
const unique = [
  ...new Map(
    candidates.map((candidate) => [candidate.repository.toLowerCase(), candidate])
  ).values(),
].sort((left, right) => left.repository.localeCompare(right.repository));
const known = new Set(state.visited.map((candidate) => candidate.repository.toLowerCase()));
const permanentFailure = new Set(
  state.failures.map((candidate) => candidate.repository.toLowerCase())
);
const newCandidates = unique.filter(
  (candidate) =>
    !known.has(candidate.repository.toLowerCase()) &&
    !permanentFailure.has(candidate.repository.toLowerCase())
);
const report = {
  schemaVersion: 1,
  discoveryDepth: 2,
  ...
  sourceDocumentsCached: state.visited.filter(
    (candidate) =>
      firstDepthRepositories.has(candidate.repository.toLowerCase()) &&
      candidate.readmePath &&
      fs.existsSync(cachePath(candidate))
  ).length,
  candidates: unique,
  newCandidates,
  edges,
...
state.edges.push(
  ...edges.filter(
    (edge) =>
      !state.edges.some(
        (existing) =>
          existing.fromRepository === edge.fromRepository &&
          existing.toRepository === edge.toRepository
      )
  )
);
