...
const included = new Set(lock.sources.filter((source) => source.status === 'included').map((source) => source.sourceId));
const errors = [];
const seen = new Set();
...
for (const batch of status.batches ?? []) {
  ...
  for (const sourceId of batch.sourceIds) {
    if (!included.has(sourceId) || seen.has(sourceId)) errors.push(`sourceIdが不正または重複です: ${sourceId}`);
    seen.add(sourceId);
  ...
  if (batch['translation-validated'] === 'completed' && Object.values(batch.pages).some((page) => page['translation-validated'] !== 'completed'))
    errors.push(`バッチ翻訳検査状態がページと一致しません: ${batch.batchNumber}`);
}
for (const sourceId of included) if (!seen.has(sourceId)) errors.push(`翻訳バッチにありません: ${sourceId}`);
for (const file of jaFiles) {
  const content = fs.readFileSync(file, 'utf8');
  const sourceId = content.match(/^licenseSource:\s*"?([^"\n]+)"?$/m)?.[1];
  const page = status.batches.flatMap((batch) => batch.pages?.[sourceId] ? [batch.pages[sourceId]] : [])[0];
...
console.log(`Awesome pipeline status: OK (${status.batches.length} batches, ${jaFiles.length}/${included.size} translated pages)`);
