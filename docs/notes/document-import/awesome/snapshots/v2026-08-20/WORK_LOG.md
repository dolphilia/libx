...
## 2026-08-30（内部リンク完全性の再検証）

- 完了したバッチ: 内部アンカー・相対リンクを含む強化後の翻訳検査を履歴版365ページへ適用した。リンク列が対応する11ページは英語定本のリンク先へ機械同期し、リンク数が異なる7ページは英語定本が新版とバイト一致することを確認したうえで、修復済みの同一原文対応訳を同期した。計18ページの修復後、完全性必須検査は365/365で合格した。
- 証拠への影響: 旧包括承認と現キューを項目ID・証拠ハッシュで再照合した結果、継承可能な承認は317件から305件へ減り、保留は100件から112件へ増えた。`FINAL_REVIEW_QUEUE.json`、`FINAL_REVIEW_RESULTS.json`、`HISTORICAL_REVIEW_DIFF_CLASSIFICATION.json`、`MACHINE_AUDIT.json`、`BATCH_STATUS.json`、公開メタデータを再生成し、旧承認を自動転記していない。
- 最後に成功した検査: 履歴版のrecords、publish `--check`、single-app、translation complete、pipeline、machine audit `--check`、最終レビュー同期、asset検査、Awesome 2,082ページビルド、選択的統合ビルド、links、integrity、deployment-assets、runtime 86/86、smoke 12/12、`git diff --check`。
- 公開判断: 2026-08-23のProduction記録は当時の成果物に対する履歴証明として保持する。今回の修復後成果物は未公開であり、現行Preview・Production証明として再利用しない。
- 次に実行する一手: 保留112件を人手レビューし、新版759件とともにPreview確認へ進める。

## 2026-08-30（タイトル・序文正規化）

- 完了したバッチ: 英日365組・730文書を正規化し、うち347件は最新版と冒頭・保持本文ハッシュが一致する判断を継承した。重要状態10件を概要へ保持し、未解決・例外0とした。
- 影響確認: 固定入力からの再importで241件の既存ドリフトを検出し、保持本文を変更せず診断JSONへ固定した。
- 最後に成功した検査: 序文監査730/730、翻訳365/365、canonical 365件、publish、機械監査419件、自動証拠レビュー419/419、履歴版単独復元検査、ビルド・全回帰検査。
