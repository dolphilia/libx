...
2. **dist/バックアップ**: 既存の`dist/`ディレクトリを`.backups/build-integrated/`に保存
3. **個別ビルド**: 各プロジェクトを順次`pnpm build`
4. **出力統合**: すべてのビルド結果を`dist/`に統合
5. **ベースパス修正**: HTML内のパス書き換え
6. **サイドバーコピー**: JSONファイルを適切な場所に配置

**バックアップについて**: ビルド前に既存の`dist/`ディレクトリが自動的にバックアップされます。詳細は[バックアップ運用ガイド](BACKUP_OPERATIONS_GUIDE.md)を参照してください。
