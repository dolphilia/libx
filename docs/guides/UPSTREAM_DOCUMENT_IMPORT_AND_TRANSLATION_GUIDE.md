...
- 既定と異なる出典を使うページだけへの`licenseSource`
- 固定版原典とライセンス情報のプロジェクト設定への集約
...
- Frontmatterキーと、指定されている場合の`licenseSource`
- コードフェンスとインラインコード
- HTMLタグとアンカー
- URL
- API識別子、型、マクロ、Cキーワード
...
英語定本と翻訳で、ファイル集合、コード、URL、HTMLアンカー、API識別子、明示された`licenseSource`、保護トークン残留を比較する。通常はプロジェクトの`defaultSource`を使い、同じ値を全ページへ重複記述しない。欠落を許容する途中検査と、公開前の欠落不可検査を分ける。
