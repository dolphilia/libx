...
🔄 自動ロールバックを開始します...
  ✅ ディレクトリ削除: /v2/de
  ✅ ディレクトリ削除: /v1/de  
  ✅ ファイル復元: project.config.jsonc
  ✅ ファイル復元: projects.config.jsonc
...
`apps/[project-name]/src/config/project.config.jsonc`を編集し、新しい言語を追加します。
...
子appは `reference/web` のようなIDで指定する。グループ名だけ、または曖昧な子名だけでは指定しない。`--dry-run` で変更範囲を確認できる。標準の文書配置を持つ子では、従来の追加操作がその子の設定・文書に適用される。

グループの言語選択は各子の実ページ目録から計算するため、一子の設定だけを変更して他の子も翻訳済みとは表示しない。同じ文書の翻訳がない場合は対応する一覧へ誘導する。追加後は[グループの統合ビルドとプレビュー](./BUILD_AND_DEPLOYMENT_GUIDE.md)で子をまたぐ切替も確認する。Awesomeは専用の翻訳・公開パイプラインを使用する。
