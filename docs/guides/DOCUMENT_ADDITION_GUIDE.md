...
- ✅ **テンプレート**: 標準Markdownを既定とし、`--format=mdx`でMDXも生成
...
| `--dry-run` | 変更予定だけを表示 | `--dry-run` |
| `--format=md\|mdx` | 文書形式を指定（既定は`md`） | `--format=mdx` |
...
- **形式**: `[番号]-[名前].md`または`[番号]-[名前].mdx`
- **番号**: 2桁のゼロパディング（01, 02, 03...）
- **名前**: URL-friendly形式（英数字とハイフン）
- **選択**: 通常は`.md`、UIコンポーネントや式が必要な場合だけ`.mdx`
...
#### ステップ2: Markdownファイルの作成

```bash
# 新しいドキュメントファイルを作成
cd 03-troubleshooting
touch 01-common-issues.md
```

#### ステップ3: Markdownファイルの編集
...
- [ ] 適切なディレクトリ構造での配置
- [ ] カテゴリディレクトリの正しい番号付け
- [ ] Markdown／MDXファイルの正しい命名（番号-名前.md／.mdx）
...
通常のMarkdownとMDXのどちらでも、注意書きは次の移植可能な記法を使えます。`NOTE`、`TIP`、`WARNING`、`DANGER`だけが有効です。角括弧の後ろの表示ラベルは本文の言語で記述します。

```markdown
> [!WARNING] 警告
> この操作の前にバックアップを作成してください。
```

libx以外のMarkdown処理系では通常の引用として本文が残ります。資料からの引用には`[!TYPE]`を付けず、通常の`>`を使ってください。

用語と説明の意味的な対応が重要な場合は、標準Markdown内で利用できるHTMLの定義リストを使います。

```html
<dl>
  <dt>定本</dt>
  <dd>翻訳の基準にする原文側の文書です。</dd>
</dl>
```
...
<Alert title="注意点" variant="warning">
  この機能は実験的な機能です。
</Alert>
```
...
通常の文書配置を持つ子appは、プロジェクト引数へ `reference/web` のようなIDを指定する。本文はその子の `src/content/docs/<version>/<lang>/` に置く。グループ親には本文を配置しない。

公開URLは子の物理パスと別に解決される。グループ内で同じ公開URL・文書IDが衝突しないよう確認し、子の更新後にグループ目録を統合する。カテゴリは所有appの別名ではなく、同じカテゴリへ複数の子が文書を持てる。

Awesomeの本文は取得元の所有割当と専用生成器が管理するため、通常の文書追加CLIを使用しない。専用取得元を追加・移動する際は、所有割当、出典レジストリ、経路・検索・翻訳目録も更新する。[入れ子実装計画](../plans/NESTED_APP_GROUPS_IMPLEMENTATION_PLAN.md)と[統合ビルドガイド](./BUILD_AND_DEPLOYMENT_GUIDE.md)を参照。
