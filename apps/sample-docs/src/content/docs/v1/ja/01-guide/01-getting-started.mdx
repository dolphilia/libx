...
libx では、各アプリケーションごとに `apps/<project-name>/src/content/docs/<version>/<lang>/` という物理構造を持たせ、`project.config.jsonc` に記述したバージョン情報を元に UI を組み立てます。`sample-docs` プロジェクトの構造は次のとおりです。

```
apps/sample-docs/
├── src/
│   ├── config/project.config.jsonc   # バージョン・言語定義を集約
│   └── content/docs/
│       ├── v1/
│       │   └── ja/01-guide/...
│       └── v2/
...
`project.config.jsonc` の `versioning.versions` で ID、表示名、リリース日、最新フラグを管理することで、ヘッダーのバージョンセレクターや URL の自動切替が機能します。新バージョンを追加するときは `node scripts/create-version.js sample-docs v3` を実行すると、すべての言語/カテゴリが一括で生成されます。
...
sample-docs はバージョン優先（`/docs/sample-docs/<version>/<lang>/...`）の URL パターンを採用しています。`@docs/content-utils` の `getSidebarAsync` は既定の `version-first` パターンで、言語ごと・バージョンごとのサイドバー JSON（`apps/sample-docs/public/sidebar/sidebar-ja-v2.json` など）を生成します。

## バージョニングの運用ポイント

1. **構造を変えずに並行更新**: v1 のカテゴリ構造をそのまま v2 にコピーし、v2 で新規セクションを育てる。
2. **設定ファイルは JSON**: `project.config.jsonc` を直接編集するよりも `scripts/create-version.js` や `scripts/add-language.js` を使い、バックアップ付きで安全に更新する。
...
- **[バージョン管理](/ja/v2/guide/version-management)** で `project.config.jsonc` と自動化スクリプトの関係を把握
