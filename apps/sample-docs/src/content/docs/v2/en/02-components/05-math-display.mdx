...
import MathDisplay from '@docs/ui/math';

# MathDisplay Component

The MathDisplay component from `@docs/ui/math` converts KaTeX expressions to HTML during server rendering, so no extra client-side scripts are required. You can mix mathematical notation with narrative text just like in academic papers.
...
<MathDisplay
  block
  expression={String.raw`f: \R^n \to \R, \; \vect{x} \mapsto \sum_{i=1}^n x_i^2`}
  options={{ macros }}
/>
```
