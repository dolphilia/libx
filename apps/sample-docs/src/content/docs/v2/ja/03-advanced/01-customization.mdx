...
プロジェクト固有の設定は `apps/<project-name>/src/config/project.config.jsonc` にまとまっています。JSON 形式なので、コードから読み込む場合は `@docs/project-config` の `getLegacyProjectConfig` などを利用します。
...
`basic.baseUrl` は実際の公開 URL (`/docs/sample-docs`) を左右し、`translations.categories` を編集すると自動生成されるサイドバーにも反映されます。値を直接書き換える際は `.backups/project-config-*/` に自動保存されたバックアップを確認しつつ、`node scripts/validate-category-structure.js` で多言語の整合性を検証してください。
...
- **[自動化](/v2/ja/03-advanced/02-automation)**: ビルドとデプロイの自動化
- **[デプロイ](/v2/ja/03-advanced/03-deployment)**: 本番環境へのデプロイ戦略
- **[リファレンス](/v2/ja/04-reference/01-frontmatter)**: 詳細なAPIリファレンス
