...
`paths.baseUrl` は実際の公開 URL (`/docs/sample-docs`) を左右し、`translations.categories` を編集すると自動生成されるサイドバーにも反映されます。値を直接書き換える際は `.backups/project-config-*/` に自動保存されたバックアップを確認しつつ、`node scripts/validate-category-structure.js` で多言語の整合性を検証してください。
