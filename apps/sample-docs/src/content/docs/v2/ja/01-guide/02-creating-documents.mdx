...
sample-docs の例：
- `apps/sample-docs/src/content/docs/v2/ja/01-guide/02-creating-documents.mdx`
- `apps/sample-docs/src/content/docs/v2/ja/02-components/02-icons.mdx`
...
新しいドキュメントを作成する最も簡単な方法は、モノレポ直下でスクリプトを実行することです：
...
手動でドキュメントを作成したい場合は、以下の手順に従ってください（バックアップや自動採番が行われないため、基本的にはスクリプト利用を推奨します）。
...
```bash
touch apps/sample-docs/src/content/docs/v2/ja/01-guide/06-my-new-guide.mdx
```
...
- **[ドキュメントの編集](/v2/ja/01-guide/03-editing-documents)**：コンテンツの編集と更新のベストプラクティス
- **[コンテンツの整理](/v2/ja/01-guide/04-organizing-content)**：ドキュメントの構造化と分類方法
- **[コンポーネントの使用](/v2/ja/02-components/01-overview)**：Icon/Tabs/Sidebar などのコンポーネント例を確認
