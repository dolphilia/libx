...
This library provides the functionality of the debug interface ([§4.7](/docs/lua/v5-5-1/en/03-c-api/17-debug-interface/#4.7)) to Lua programs.
...
All functions in this library are provided inside the <a id="pdf-debug"></a>`debug` table. All functions that operate over a thread have an optional first argument which is the thread to operate over. The default is always the current thread.
...
Returns the current hook settings of the thread, as three values: the current hook function, the current hook mask, and the current hook count, as set by the [`debug.sethook`](/docs/lua/v5-5-1/en/05-standard-library/12-debug-library/#pdf-debug.sethook) function.
...
The returned table can contain all the fields returned by [`lua_getinfo`](/docs/lua/v5-5-1/en/03-c-api/17-debug-interface/#lua_getinfo), with the string `what` describing which fields to fill in. The default for `what` is to get all information available, except the table of valid lines. The option '`f`' adds a field named `func` with the function itself. The option '`L`' adds a field named `activelines` with the table of valid lines, provided the function is a Lua function. If the function has no debug information, the table is empty.

For instance, the expression `debug.getinfo(1,"n").name` returns a name for the current function, if a reasonable name can be found, and the expression `debug.getinfo(print)` returns a table with all available information about the [`print`](/docs/lua/v5-5-1/en/05-standard-library/03-basic-functions/#pdf-print) function.
...
The function returns **fail** if there is no variable with the given index, and raises an error when called with a level out of range. (You can call [`debug.getinfo`](/docs/lua/v5-5-1/en/05-standard-library/12-debug-library/#pdf-debug.getinfo) to check whether the level is valid.)
...
Returns the registry table (see [§4.3](/docs/lua/v5-5-1/en/03-c-api/04-registry/#4.3)).
...
When called without arguments, [`debug.sethook`](/docs/lua/v5-5-1/en/05-standard-library/12-debug-library/#pdf-debug.sethook) turns off the hook.
...
See [`debug.getlocal`](/docs/lua/v5-5-1/en/05-standard-library/12-debug-library/#pdf-debug.getlocal) for more information about variable indices and names.
