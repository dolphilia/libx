...
The string library provides all its functions inside the table <a id="pdf-string"></a>`string`. It also sets a metatable for strings where the `__index` field points to the `string` table. Therefore, you can use the string functions in object-oriented style. For instance, `string.byte(s,i)` can be written as `s:byte(i)`.
...
Returns a string containing a binary representation (a *binary chunk*) of the given function, so that a later [`load`](/docs/lua/v5-5-1/en/05-standard-library/03-basic-functions/#pdf-load) on this string returns a copy of the function (but with new upvalues). If `strip` is a true value, the binary representation may not include all debug information about the function, to save space.

Functions with upvalues have only their number of upvalues saved. When (re)loaded, those upvalues receive fresh instances. (See the [`load`](/docs/lua/v5-5-1/en/05-standard-library/03-basic-functions/#pdf-load) function for details about how these upvalues are initialized. You can use the debug library to serialize and reload the upvalues of a function in a way adequate to your needs.)

---

## <a id="pdf-string.find"></a>`string.find (s, pattern [, init [, plain]])`

Looks for the first match of `pattern` (see [§6.5.1](/docs/lua/v5-5-1/en/05-standard-library/06-string-manipulation/#6.5.1)) in the string `s`. If it finds a match, then `find` returns the indices of `s` where this occurrence starts and ends; otherwise, it returns **fail**. A third, optional numeric argument `init` specifies where to start the search; its default value is 1 and can be negative. A **true** as a fourth, optional argument `plain` turns off the pattern matching facilities, so the function does a plain "find substring" operation, with no characters in `pattern` being considered magic.
...
The specifier `s` expects a string; if its argument is not a string, it is converted to one following the same rules of [`tostring`](/docs/lua/v5-5-1/en/05-standard-library/03-basic-functions/#pdf-tostring). If the specifier has any modifier, the corresponding string argument should not contain embedded zeros.

The specifier `p` formats the pointer returned by [`lua_topointer`](/docs/lua/v5-5-1/en/03-c-api/15-functions-and-types-to-type/#lua_topointer). That gives a unique string identifier for tables, userdata, threads, strings, and functions. For other values (numbers, nil, booleans), this specifier results in a string representing the pointer `NULL`.
...
Looks for the first *match* of the `pattern` (see [§6.5.1](/docs/lua/v5-5-1/en/05-standard-library/06-string-manipulation/#6.5.1)) in the string `s`. If it finds one, then `match` returns the captures from the pattern; otherwise it returns **fail**. If `pattern` specifies no captures, then the whole match is returned. A third, optional numeric argument `init` specifies where to start the search; its default value is 1 and can be negative.

---

## <a id="pdf-string.pack"></a>`string.pack (fmt, v1, v2, ···)`

Returns a binary string containing the values `v1`, `v2`, etc. serialized in binary form (packed) according to the format string `fmt` (see [§6.5.2](/docs/lua/v5-5-1/en/05-standard-library/06-string-manipulation/#6.5.2)).

---

## <a id="pdf-string.packsize"></a>`string.packsize (fmt)`

Returns the length of a string resulting from [`string.pack`](/docs/lua/v5-5-1/en/05-standard-library/06-string-manipulation/#pdf-string.pack) with the given format. The format string cannot have the variable-length options '`s`' or '`z`' (see [§6.5.2](/docs/lua/v5-5-1/en/05-standard-library/06-string-manipulation/#6.5.2)).
...
## 6.5.1 – <a id="6.5.1"></a>Patterns

Patterns in Lua are described by regular strings, which are interpreted as patterns by the pattern-matching functions [`string.find`](/docs/lua/v5-5-1/en/05-standard-library/06-string-manipulation/#pdf-string.find), [`string.gmatch`](/docs/lua/v5-5-1/en/05-standard-library/06-string-manipulation/#pdf-string.gmatch), [`string.gsub`](/docs/lua/v5-5-1/en/05-standard-library/06-string-manipulation/#pdf-string.gsub), and [`string.match`](/docs/lua/v5-5-1/en/05-standard-library/06-string-manipulation/#pdf-string.match). This section describes the syntax and the meaning (that is, what they match) of these strings.
...
## 6.5.2 – <a id="6.5.2"></a>Format Strings for Pack and Unpack

The first argument to [`string.pack`](/docs/lua/v5-5-1/en/05-standard-library/06-string-manipulation/#pdf-string.pack), [`string.packsize`](/docs/lua/v5-5-1/en/05-standard-library/06-string-manipulation/#pdf-string.packsize), and [`string.unpack`](/docs/lua/v5-5-1/en/05-standard-library/06-string-manipulation/#pdf-string.unpack) is a format string, which describes the layout of the structure being created or read.
...
For options "`!n`", "`sn`", "`in`", and "`In`", `n` can be any integer between 1 and 16. All integral options check overflows; [`string.pack`](/docs/lua/v5-5-1/en/05-standard-library/06-string-manipulation/#pdf-string.pack) checks whether the given value fits in the given size; [`string.unpack`](/docs/lua/v5-5-1/en/05-standard-library/06-string-manipulation/#pdf-string.unpack) checks whether the read value fits in a Lua integer. For the unsigned options, Lua integers are treated as unsigned values too.
...
All padding is filled with zeros by [`string.pack`](/docs/lua/v5-5-1/en/05-standard-library/06-string-manipulation/#pdf-string.pack) and ignored by [`string.unpack`](/docs/lua/v5-5-1/en/05-standard-library/06-string-manipulation/#pdf-string.unpack).
