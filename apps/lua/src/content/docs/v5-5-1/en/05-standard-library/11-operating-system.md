...
This library is implemented through table <a id="pdf-os"></a>`os`.
...
If the `time` argument is present, this is the time to be formatted (see the [`os.time`](/docs/lua/v5-5-1/en/05-standard-library/11-operating-system/#pdf-os.time) function for a description of this value). Otherwise, `date` formats the current time.
...
Returns the difference, in seconds, from time `t1` to time `t2` (where the times are values returned by [`os.time`](/docs/lua/v5-5-1/en/05-standard-library/11-operating-system/#pdf-os.time)). In POSIX, Windows, and some other systems, this value is exactly `t2`*-*`t1`.
...
If the optional second argument `close` is true, the function closes the Lua state before exiting (see [`lua_close`](/docs/lua/v5-5-1/en/03-c-api/07-functions-and-types-a-c/#lua_close)).
...
Returns the current local time when called without arguments, or a time representing the local date and time specified by the given table. This table must have fields `year`, `month`, and `day`, and may have fields `hour` (default is 12), `min` (default is 0), `sec` (default is 0), and `isdst` (default is **nil**). Other fields are ignored. For a description of these fields, see the [`os.date`](/docs/lua/v5-5-1/en/05-standard-library/11-operating-system/#pdf-os.date) function.
...
The returned value is a number, whose meaning depends on your system. In POSIX, Windows, and some other systems, this number counts the number of seconds since some given start time (the "epoch"). In other systems, the meaning is not specified, and the number returned by `time` can be used only as an argument to [`os.date`](/docs/lua/v5-5-1/en/05-standard-library/11-operating-system/#pdf-os.date) and [`os.difftime`](/docs/lua/v5-5-1/en/05-standard-library/11-operating-system/#pdf-os.difftime).

When called with a table, `os.time` also normalizes all the fields documented in the [`os.date`](/docs/lua/v5-5-1/en/05-standard-library/11-operating-system/#pdf-os.date) function, so that they represent the same time as before the call but with values inside their valid ranges.
...
When possible, you may prefer to use [`io.tmpfile`](/docs/lua/v5-5-1/en/05-standard-library/10-input-and-output/#pdf-io.tmpfile), which automatically removes the file when the program ends.
