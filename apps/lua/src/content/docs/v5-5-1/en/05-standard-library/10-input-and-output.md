...
When using implicit file handles, all operations are supplied by table <a id="pdf-io"></a>`io`. When using explicit file handles, the operation [`io.open`](/docs/lua/v5-5-1/en/05-standard-library/10-input-and-output/#pdf-io.open) returns a file handle and then all operations are supplied as methods of the file handle.

The metatable for file handles provides metamethods for `__gc` and `__close` that try to close the file when called.

The table `io` also provides three predefined file handles with their usual meanings from C: <a id="pdf-io.stdin"></a>`io.stdin`, <a id="pdf-io.stdout"></a>`io.stdout`, and <a id="pdf-io.stderr"></a>`io.stderr`. The I/O library never closes these files.
...
Similar to [`io.input`](/docs/lua/v5-5-1/en/05-standard-library/10-input-and-output/#pdf-io.input), but operates over the default output file.
...
When closing a file handle created with [`io.popen`](/docs/lua/v5-5-1/en/05-standard-library/10-input-and-output/#pdf-io.popen), [`file:close`](/docs/lua/v5-5-1/en/05-standard-library/10-input-and-output/#pdf-file:close) returns the same values returned by [`os.execute`](/docs/lua/v5-5-1/en/05-standard-library/11-operating-system/#pdf-os.execute).
...
will iterate over all characters of the file, starting at the current position. Unlike [`io.lines`](/docs/lua/v5-5-1/en/05-standard-library/10-input-and-output/#pdf-io.lines), this function does not close the file when the loop ends.
