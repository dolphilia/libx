...
You can query the metatable of any value using the [`getmetatable`](/docs/lua/v5-5-1/en/05-standard-library/03-basic-functions/#pdf-getmetatable) function. Lua queries metamethods in metatables using a raw access (see [`rawget`](/docs/lua/v5-5-1/en/05-standard-library/03-basic-functions/#pdf-rawget)).

You can replace the metatable of tables using the [`setmetatable`](/docs/lua/v5-5-1/en/05-standard-library/03-basic-functions/#pdf-setmetatable) function. You cannot change the metatable of other types from Lua code, except by using the debug library ([§6.11](/docs/lua/v5-5-1/en/05-standard-library/12-debug-library/#6.11)).

Tables and full userdata have individual metatables, although multiple tables and userdata can share their metatables. Values of all other types share one single metatable per type; that is, there is one single metatable for all numbers, one for all strings, etc. By default, a value has no metatable, but the string library sets a metatable for the string type (see [§6.5](/docs/lua/v5-5-1/en/05-standard-library/06-string-manipulation/#6.5)).
...
- **`__band`**: the bitwise AND (`&`) operation. Behavior similar to the addition operation, except that Lua will try a metamethod if any operand is neither an integer nor a float coercible to an integer (see [§3.4.3](/docs/lua/v5-5-1/en/02-language/10-expressions/#3.4.3)).
...
- **`__len`**: the length (`#`) operation. If the object is not a string, Lua will try its metamethod. If there is a metamethod, Lua calls it with the object as argument, and the result of the call (always adjusted to one value) is the result of the operation. If there is no metamethod but the object is a table, then Lua uses the table length operation (see [§3.4.7](/docs/lua/v5-5-1/en/02-language/10-expressions/#3.4.7)). Otherwise, Lua raises an error.
...
In addition to the previous list, the interpreter also respects the following keys in metatables: `__gc` (see [§2.5.3](/docs/lua/v5-5-1/en/02-language/05-garbage-collection/#2.5.3)), `__close` (see [§3.3.8](/docs/lua/v5-5-1/en/02-language/09-statements/#3.3.8)), `__mode` (see [§2.5.4](/docs/lua/v5-5-1/en/02-language/05-garbage-collection/#2.5.4)), and `__name`. (The entry `__name`, when it contains a string, may be used by [`tostring`](/docs/lua/v5-5-1/en/05-standard-library/03-basic-functions/#pdf-tostring) and in error messages.)

For the unary operators (negation, length, and bitwise NOT), the metamethod is computed and called with a dummy second operand, equal to the first one. This extra operand is only to simplify Lua's internals (by making these operators behave like a binary operation) and may be removed in future versions. For most uses this extra operand is irrelevant.

Because metatables are regular tables, they can contain arbitrary fields, not only the event names defined above. Some functions in the standard library (e.g., [`tostring`](/docs/lua/v5-5-1/en/05-standard-library/03-basic-functions/#pdf-tostring)) use other fields in metatables for their own purposes.

It is a good practice to add all needed metamethods to a table before setting it as a metatable of some object. In particular, the `__gc` metamethod works only when this order is followed (see [§2.5.3](/docs/lua/v5-5-1/en/02-language/05-garbage-collection/#2.5.3)). It is also a good practice to set the metatable of an object right after its creation.
