...
Arch ベースのプロジェクトには Linux カーネルを使うものと使わないものがあり、Arch 愛好者の選択肢をさらに広げています。

### Linux

> Arch Linux に強く影響を受けた Linux ディストリビューション。
...
> Arch Linux を基盤とする非 Linux プロジェクト。
...
> 一般用途の Arch Linux ベース Linux ディストリビューション。
...
> 特定の目的を持つ Arch Linux ベース Linux ディストリビューション。
...
## SystemD を使わない Arch ベースのディストリビューション
...
この節では、これらすべてのヘルパーを機能別に分類して扱います。
...
> これまでのカテゴリに属さない、Arch Linux に関連または着想を得たプロジェクト。
...
> リストには含まれていたものの、廃止または長期間更新されていないプロジェクト。
...
貢献を歓迎します。 [contribution guidelines](https://github.com/PandaFoss/Awesome-Arch/blob/ed749c354881af6727101aa1c650215fcff9b8c4/contributing.md) first.

少額の寄付（こっそり、コーヒーをごちそうして… ☕️ ）でリストの保守にも貢献できます:
...
以下の素晴らしい方々に感謝します ([emoji key](https://allcontributors.org/docs/en/emoji-key)):
...
法律で可能な限り、Panda Foss はこの作品に関するすべての著作権および関連・隣接権を放棄しています。
