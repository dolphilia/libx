...
> **Eddystone**は、適切なタイミングで文脈に応じた情報を提供し、アプリや端末をより賢く動作させるために現実世界へ情報を付与するプラットフォームです。
...
* [Eddystone GATT Configuration Service & Google Nearby API and Proximity API](* [](https://github.com/NordicSemiconductor/Android-nRF-Beacon-for-Eddystone)（ノルディック半導体による）
...
## Physical Web <a id="physical-web"></a>

> **Physical Web**は、必要なときに即座に操作できるというウェブ本来の力を引き出す取り組みです。自動販売機、ポスター、玩具、バス停、レンタカーなどのスマートデバイスへ近づくだけで、事前にアプリをダウンロードせず、タップだけで利用できることを目指します。

* [Physical Web - Walk up and use anything](http://google.github.io/physical-web/) - 公式GitHubリポジトリ
...
* [IEEE: Enabling the Internet of Things](https://web.eecs.umich.edu/~prabal/teaching/resources/eecs582/want15iot.pdf) R. Want, B. Schilit, S. Jensonによる
...
* [Beacons 101-- Getting Started with the Google Beacon Platform](https://www.youtube.com/watch?v=0QeY9FueMow) - Ubiquity Dev Summit 2016の動画
* [Get Started with Beacons](https://developers.google.com/beacons/get-started) - このガイドは、ユーザーにプロキシティベースの体験を提供するためにBluetooth Low Energy (BLE) ベーコンを使用開始するための手順を説明しています.
* [Proximity Beacon API](https://developers.google.com/beacons/proximity/guides) - BLEベーコンに関連するデータをRESTインターフェースを使って管理できるクラウドサービス
* [Nearby](https://developers.google.com/nearby/) - 近くのデバイスと人々との間のシンプルな相互作用を構築


## AppleのiBeacon資料 <a id="ibeacon-resources-by-apple"></a>

> **iBeacon for Developers** スポーツイベント会場への来場者の歓迎から、近くの博物館展示の案内まで、iBeaconは位置認識の新しい可能性と、iOS端末とiBeaconハードウェア間の多様なインタラクションを実現します。
...
# 活用例と便利なアプリ <a id="hacks--cool-apps"></a>

* [Empowering vision impaired people to navigate the world independently](https://www.wayfindr.net) (オープンスタンダード)
* [Google Glass & Beacons](https://github.com/tmwagency/Glasstimote)
* [10 awesome things you can do today with iBeacons](http://blog.twocanoes.com/post/68861362715/10-awesome-things-you-can-do-today-with-ibeacons) (Twocanoesによる)
* [PunchClock](https://github.com/panicinc/PunchClock) iOS 7以降向けの出入りトラッキングアプリ（iBeaconとGeofencingを使用）
* [The Geofancy iOS app](https://github.com/LocativeHQ/ios-app) 自動化された家をよりよく活用できるようにサポートします。地理的フェンス、あなたの手にあるiBeacons。
...
* [ScanBeacon](https://github.com/RadiusNetworks/scanbeacon-gem) - IOBluetooth（Mac OS X）またはBlueGiga BLE112端末（Mac／Linux）を使ってBLE AdvertisementをスキャンするRuby gem。
...
* [HiBeacons](https://github.com/nicktoumpelis/HiBeacons) Swiftで書かれた良いiBeaconデモアプリ。
...
* [JMCiBeaconManager](https://github.com/izotx/JMCBeaconManager) - 周囲のbeaconsを検出するためのiBeaconマネージャークラス。
* [BeaconKit](https://github.com/igor-makarov/BeaconKit) - CoreBluetoothを使用したbeacon検出フレームワーク。Eddystone-UID、Eddystone-URL、AltBeaconをサポート。
...
* [RABeaconManager](https://github.com/reelyactive/ble-ios-sdk) RABeaconManagerは、前処理およびバックグラウンドでBluetooth BeaconおよびiBeaconを検出できる使いやすいライブラリ。


## Stack Overflow Q&A <a id="stackoverflow-qa"></a>
...
* [Chrome App to advertise Eddystone packets](https://github.com/google/eddystone/tree/master/tools/eddystone-chrome-app-sample) - [Eddystone Advertising Library](https://github.com/google/eddystone/tree/master/libraries/javascript/eddystone-advertising)を使用します
...
* [Android Lolipop Bluetooth Low Energy Enhancements.](https://developer.android.com/about/versions/android-5.0.html) OSレベルのスキャンフィルタおよびペリフェラルモードをサポート。
...
* [Android beacon library based on AltBeacon.](https://github.com/AltBeacon/android-beacon-library) iBeaconデバイスとの互換性のためにカスタムbeaconパーサーを使用。
* [BeaconKeeper](https://github.com/m039/beacon-keeper) バックグラウンドでのiBeaconの位置特定に必要なシンプルなライブラリ。
...
* [reelyactive-ble-android-sdk](https://github.com/reelyactive/ble-android-sdk) このSDKは、beaconをスキャンし、beaconとして広告を送信できるようにします。

## ビーコンスキャナーアプリ <a id="beacon-scanner-apps-1"></a>
...
* [Beacon Scanner & Logger](https://github.com/justinodwyer/Beacon-Scanner-and-Logger) BLE Beacon/iBeaconをスキャンし、結果をファイルにログ出力するAndroidアプリ。
...
* [Beacon OSX](https://github.com/mttrb/BeaconOSX) - MavericksにおけるiBeaconの実装
* [Electron Physical Web Scan](https://github.com/dermike/electron-physical-web-scan) - Mac OSX用のデスクトップアプリで物理ウェブ（Eddystone）Bluetoothベーコンをスキャン
* [Electron Slide Beacon](https://github.com/dermike/electron-slide-beacon) - このアプリを使ってMacからリンクを共有し、Eddystone URL（物理ウェブ）Bluetoothベーコンとしてそれをブロードキャストします
* [BeaconKit](https://github.com/igor-makarov/BeaconKit) - CoreBluetoothを使用するSwift製ビーコン検出フレームワーク。Eddystone-UID、Eddystone-URL、AltBeacon、iBeaconをサポート
...
* [Universal Bluetooth Beacon Library](https://github.com/andijakl/universal-beacon) - EddystoneおよびiBeaconベーコンと通信するためのオープンソースライブラリとアプリへのリンク
...
* [nRF Master Control Panel (BLE)](https://play.google.com/store/apps/details?id=no.nordicsemi.android.mcp) nRFマスターコントロールパネルは、Bluetooth Smart（BLE）デバイスをスキャン・探索し、それらと通信できる強力な汎用ツールです
* [LightBlue Mac OSX](https://itunes.apple.com/de/app/lightblue/id639944780?mt=12)
[(or iOS)](https://itunes.apple.com/us/app/lightblue-bluetooth-low-energy/id557428110?mt=8) LightBlueは、Bluetooth 4.0 Low Energy（Bluetooth SmartまたはBluetooth Light）を使用するすべてのデバイスをテストできます
* [BlueSpeed for iOS by Punch Through](https://itunes.apple.com/us/app/bluespeed/id579118786?mt=8) 2つのiOSデバイス間のBluetooth LE速度テストを実行

## Web Bluetooth API

* [Web Bluetooth Intro](https://dev.opera.com/articles/web-bluetooth-intro/) オペラによる
...
* [Wired](http://www.wired.com/2013/12/4-use-cases-for-ibeacon-the-most-exciting-tech-you-havent-heard-of/) なぜAppleのiBeaconがインタラクションデザインを変えるのか（4つの理由）
* [Wi-Fi Aware™](http://www.wi-fi.org/discover-wi-fi/wi-fi-aware) - Wi-Fi Aware™は、Wi-Fiアライアンスが提供する新しい認定プログラムで、リアルタイムかつエネルギー効率の高い発見メカニズムを導入し、即時的に豊かな「ここにいる今」の体験にアクセスできるようになります


## ベンダー主導のビーコン標準化 <a id="vendor-driven-beacon-standardization"></a>

* [BeaconCtrl](https://github.com/upnext/BeaconCtrl) BeaconCtrl オープンソースプラットフォームを使って大規模なベーコン展開の設定と管理
* [The Open and Interoperable Proximity Beacon Specification](http://altbeacon.org/)

---

Bluetooth、Apple、Google、iBeacon、Eddystoneなどの商標は、それぞれの商標権者に帰属します。

# ライセンス <a id="license"></a>

[![CC0](http://i.creativecommons.org/p/zero/1.0/88x31.png)](http://creativecommons.org/publicdomain/zero/1.0/)

法律で認められる範囲で、[Square Metrics GmbH](http://www.squaremetrics.com/)は本作品に関するすべての著作権および関連する権利を放棄しています。
