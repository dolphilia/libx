...
- **文字列長は通常、コードポイントを数えて求めます。** そのため、サロゲートペアは2文字として数えられます。複数のダイアクリティカルマークが同じ文字に重なる場合もあります。`a + ̈  == ̈a   `は見た目上1文字でも長さが増えます。

- **同様に、文字列の反転も簡単ではないことがあります。** サロゲートペアとダイアクリティカルマークを一体として反転する必要があります。[ES Reverser](https://github.com/mathiasbynens/esrever)は有用な解決策を提供します。

- **大文字と小文字の対応は常に一対一とは限りません。** 次のような場合もあります。
  - 一対多：（ß → SS）
  - 文脈依存：（…Σ ↔ …ς、および …ΣΤ… ↔ …στ…）
  - ロケール依存：（I ↔ ı、および İ ↔ i）
...
*以下の文字の多くは大文字化すると一対多の対応になりますが、小文字化すべきものもあります。この一覧は分割する必要があります。*
