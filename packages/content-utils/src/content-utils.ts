 ...
 */
export async function getAvailableContent(options?: ContentOptions): Promise<ContentFile[]> {
  const projectDir = resolveProjectDir(options?.projectDir);
  const contentDir = path.join(projectDir, 'src', 'content', 'docs');
...
export async function getDefaultCardLinks(
  ...
  const guideContent = await findAllContent(lang, version, 'guide', options);
...
export async function getVersionedUrl(
  ...
  const sectionContent = allContent
    ...
    .sort((a, b) => {
      // ファイル名の番号プレフィックスでソート
      const aNumber = parseInt((a.fullPath.match(/\/(\d+)-/) || ['', '999'])[1], 10);
      const bNumber = parseInt((b.fullPath.match(/\/(\d+)-/) || ['', '999'])[1], 10);
...
export async function getDocumentTitle(
  ...
  try {
    ...
    const targetDoc = findDocEntry(docs, lang, version, section, slug);
 ...
 * バージョン別のファイル構造を取得
 */
export async function getVersionFileStructure(
  lang: LocaleKey,
  options?: ContentOptions
  ...
  // 各バージョン内でソート（優先度順）
  for (const version of Object.keys(versionStructure)) {
    versionStructure[version].sort((a, b) => {
      // セクション優先度
      const sectionPriority: Record<string, number> = { guide: 0, api: 1, examples: 2, faq: 3 };
      const aPriority = sectionPriority[a.section] ?? 99;
      const bPriority = sectionPriority[b.section] ?? 99;
...
export async function findCorrespondingFile(
  ...
  const versionStructure = await getVersionFileStructure(currentLang, options);
